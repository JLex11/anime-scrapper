  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    res.send = function(body): Response {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        ...
        // Guardar en caché
        defaultCache.set(key, { body, headers }, ttl)
      }
...
// Preconfigurados para diferentes rutas
export const shortCache = expressCacheMiddleware({ ttl: 60 }) // 1 minuto
export const mediumCache = expressCacheMiddleware({ ttl: 300 }) // 5 minutos
export const longCache = expressCacheMiddleware({ ttl: 3600 }) // 1 hora
